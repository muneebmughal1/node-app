            ...
            steps {
                script {
                    ...
                    sshagent(credentials: [SSH_CREDENTIALS]) {
                        sh "scp -i /var/lib/jenkins/credentials/15.222.239.203 -r * ubuntu@${EC2_INSTANCE_IP}:/sample"
                    }
                    
                    // Connect to the EC2 instance and perform deployment steps
                    sshagent(credentials: [SSH_CREDENTIALS]) {
                        sh "ssh -i /var/lib/jenkins/credentials/15.222.239.203 ubuntu@${EC2_INSTANCE_IP} 'cd /sample && npm install && pm2 restart test'"
                    }
                }
            }
        }
